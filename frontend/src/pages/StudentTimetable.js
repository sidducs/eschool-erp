...
function StudentTimetable({ classId }) {
    ...
    if (loading) {
        return <div className="p-8 text-center text-slate-500 font-medium animate-pulse flex flex-col items-center"><FaClock className="mb-2 text-2xl" /> Loading timetable...</div>;
    }
        ...
        <div className="flex flex-col gap-4 animate-fadeIn max-w-7xl mx-auto p-4">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <select
                        className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 outline-none focus:ring-2 focus:ring-purple-500"
                        value={selectedDay}
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {sortedTimetable.length === 0 ? (
                                    ...
                                    sortedTimetable.map((slot) => (
                                        <tr key={slot._id} className="hover:bg-purple-50 transition-colors group">
