...
//   FaClipboardCheck, FaBook, FaUpload, FaBullhorn, // <--- Added Bullhorn Icon
...
//         api.get("/api/exams"),
//         api.get("/api/notices") // <--- Added Notices Fetch (Index 5)
//       ]);
//       setData({
...
//         notices: results[5].status === 'fulfilled' ? results[5].value.data : [] // <--- Store Notices
//       });
//       setLoading(false);
//     };
//     loadData();
...
//       // If we posted a notice, clear form data specifically
...
//           { id: "notices", label: "Notice Board", icon: FaBullhorn }, // <--- Added Notice Menu
...
//            {/* --- CLASSES & FEES MAIN VIEWS --- */}
//            {activeMenu === "classes" && (
//               <div>
//                  <div className="row g-4 mb-4">
//                     <div className="col-md-6"><div className="p-4 text-center cursor-pointer bg-white" style={styles.card} onClick={()=>setActiveMenu("classes-create")}><FaChalkboardTeacher size={32} className="text-primary mb-3"/><h6 className="fw-bold">Create Class</h6></div></div>
//                     <div className="col-md-6"><div className="p-4 text-center cursor-pointer bg-white" style={styles.card} onClick={()=>setActiveMenu("classes-assign")}><FaUsers size={32} className="text-success mb-3"/><h6 className="fw-bold">Assign Student</h6></div></div>
//                  </div>
...
//            {activeMenu === "fees" && (
//               <div className="row g-4">{[{t:"Create",m:"fees-create",i:FaMoneyBillWave},{t:"Assign",m:"fees-assign",i:FaUserGraduate},{t:"Payments",m:"fees-view",i:FaClipboardCheck}].map((x,i)=><div className="col-md-4" key={i}><div className="p-4 text-center cursor-pointer bg-white" style={styles.card} onClick={()=>setActiveMenu(x.m)}><x.i size={28} className="text-primary mb-3"/><h6 className="fw-bold">{x.t}</h6></div></div>)}</div>
//            )}
...
//                  <div className="table-responsive"><table className="table align-middle mb-0"><thead style={styles.tableHeader}><tr><th className="ps-4">Student</th><th>Total</th><th>Paid</th><th>Status</th><th>Action</th></tr></thead><tbody>{(data.fees||[]).map(f=><tr key={f._id}><td className="ps-4 fw-bold">{f.studentId?.name}</td><td>{f.totalFee}</td><td>{f.paidAmount}</td><td><span className={`badge ${f.status==='PAID'?'bg-success':'bg-warning'}`}>{f.status}</span></td><td>{f.status!=='PAID'&&<button className="btn btn-sm btn-outline-success" onClick={()=>{const amt=prompt("Amount:"); if(amt) apiAction('put','/api/fees/pay',{studentId:f.studentId._id, paidAmount:Number(amt)},"Paid",null)}}>Pay</button>}</td></tr>)}</tbody></table></div>
//               </div>
//            )}
